...
async function DeletePDP(req: any, res: any) {
    try {
        ...
        if (documento.Data_Approvazione) {
            return res.status(400).send({ message: "Non è possibile eliminare un documento approvato o scaduto." } );
        }
        ...
        res.status(500).send({ message: "Errore durante la cancellazione del documento" });
    }
}
...
async function CreatePDP(req: any, res: any) {
    try {
        ...
        res.status(500).send({ message: "Errore durante la creazione del documento" });
    }
}
    ...
    try {
        ...
        if (!await CheckAdmin(req)) {
            if (filters.Materia_Documenti_Indicatori || filters.Documento_ICFs || filters.Allegati || filters.Studente) {
                return res.status(400).send({ message: "Non è possibile filtrare i documenti con questi filtri se non si è un amministratore." });
            }

            if (req.docente.Email != filters.Docente_Email) {
                return res.status(403).send({ message: "Accesso negato: non sei il docente associato a questo/i documento/i." });
            }
            ...
            if (filters.Studente_Email && filters.Anno) {
                ...
                let isDocente = false;
                for (const idClasse of classiIds) {
                    if (await CheckDocente(req, idClasse)) {
                        isDocente = true;
                        break;
                    }
                }
        ...
        res.status(500).send({ message: "Errore nella esecuzione della richiesta dei documenti" });
    }
}
...
async function SalvaDocumentoApprovato(req: any, res: any) {
    try {
        ...
        if (!data_approvazione) {
            return res.status(400).send({ message: "Il documento non è stato approvato. Non è possibile salvarlo." });
        }
        ...
        const directoryPath = process.env.DOCUMENTI_PATH + `/${studente_email}/${anno}`;
...
async function GetDocumentoApprovato(req: any, res: any) {
    try {
        ...
        if (!anno || !studente_email) {
            return res.status(400).send({ message: "Mancano informazioni per identificare il documento da recuperare." });
        }
        ...
        if (!await CheckAdmin(req)) {
            ...
            if (!hasAccess) {
                return res.status(403).send({ message: "Accesso negato: non sei un amministratore o un docente di una classe associata a questo documento." });
            }
        }
        ...
        const pdfPath = join(process.env.DOCUMENTI_PATH + `/${studente_email}/${anno}`, 'PDP_approvato.pdf');
